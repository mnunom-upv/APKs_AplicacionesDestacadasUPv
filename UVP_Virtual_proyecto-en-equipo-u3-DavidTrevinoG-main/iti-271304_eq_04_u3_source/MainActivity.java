...
public class MainActivity extends CardboardActivity implements CardboardView.StereoRenderer {
    ...
    // Coordenadas previas para joystick
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cubeRenderer.ajustarVistaInicial();

        CardboardView cardboardView = findViewById(R.id.cardboard_view);
        cardboardView.setRenderer(this);
        setCardboardView(cardboardView, cubeRenderer);
            ...
            if (event.getRepeatCount() == 0) {
                switch (keyCode) {
                    case KeyEvent.KEYCODE_BUTTON_Y:
                        cubeRenderer.MoveUpward(5f);  // Movimiento hacia arriba
                        break;
                    case KeyEvent.KEYCODE_BUTTON_A:
                        cubeRenderer.NextPosicion();  // Siguiente posición
                        break;
                    case KeyEvent.KEYCODE_BUTTON_X:
                        cubeRenderer.MoveUpward(-5f); // Movimiento hacia abajo
                        break;
                }
            }
        }
    ...
    public boolean onGenericMotionEvent(MotionEvent event) {
        ...
        if (Dpad.isDpadDevice(event)) {
            ...
            switch (press) {
                case LEFT:
                    cubeRenderer.StrafeRight(-10f); // Movimiento a la izquierda
                    return true;
                case RIGHT:
                    cubeRenderer.StrafeRight(10f);  // Movimiento a la derecha
                    return true;
                case UP:
                    cubeRenderer.MoveForward(-10f); // Movimiento hacia adelante
                    return true;
                case DOWN:
                    cubeRenderer.MoveForward(10f);  // Movimiento hacia atrás
                    return true;
            }
        }
    ...
    public void onNewFrame(HeadTransform headTransform) {
        ...
        // Extrae los movimientos de rotación o traslación y pásalos a CubeRenderer
        cubeRenderer.actualizar_vista();
    }
    ...
    public void onSurfaceChanged(int i, int i1) {
        ...
        cubeRenderer.onSurfaceChanged(null, i1,i);
    }

    @Override
    public void onSurfaceCreated(EGLConfig eglConfig) {
        cubeRenderer.onSurfaceCreated(null, eglConfig);
        initializeScene();
        compileShaders();
    }
